...
const STATUS_TONE = { PENDING: "default", SUBMITTED: "success", LATE: "warn", GRADED: "success" };
...
function StudentAssignmentsView() {
  ...
  function load() {
    setLoading(true);
    api.get("/assignments/me").then(({ data }) => setAssignments(data)).finally(() => setLoading(false));
  }
  useEffect(load, []);
  ...
  return (
    <div>
      <PageHeader title="My Assignments" subtitle="Pending, submitted, late, and graded work." />
      {loading ? (
              ...
              <div key={a.id} className="card flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate mt-1">
                    {a.subject} · Due {new Date(a.deadline).toLocaleDateString()} · Max {a.maxMarks} marks
                  </p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {status === "PENDING" && (
                    <button onClick={() => submit(a.id)} className="text-xs text-brass hover:text-ink font-medium">
                      Submit
                    </button>
                  )}
                </div>
...
function TeacherAssignmentsView() {
  ...
  const [form, setForm] = useState({ title: "", description: "", deadline: "", maxMarks: 100, subjectId: "" });
  ...
  return (
    <div>
      <PageHeader title="Assignments" subtitle="Create assignments for your assigned subjects." />
      <form onSubmit={handleCreate} className="card space-y-4 max-w-xl">
        {created && <p className="text-sm text-success">Assignment created and rolled out to the class.</p>}
        <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="number" className="input" required value={form.maxMarks} onChange={(e) => setForm({ ...form, maxMarks: e.target.value })} />
          </div>
        </div>
        <button className="btn-primary" disabled={creating || !subjects.length}>{creating ? "Creating…" : "Create Assignment"}</button>
        {!subjects.length && <p className="text-xs text-slate">You have no assigned subjects yet — ask the Registrar to assign you one.</p>}
      </form>
    </div>
  );
}
